...
/**
 * A {@link SinkWriter} implementation for {@link RabbitMQSink} that has at-least-once
 * semantics, meaning it guarantees that outgoing message arrive at RabbitMQ at least once.
 *
 * <p>At-least-once behaviour is implemented by assigning sequence numbers to arriving messages and
 * buffering them together in the state of the writer until an ack arrives or a maximum number
 ...
 * On a checkpoint, a resend of all messages that are unacknowledged and at least
 * {@code resendIntervalMilliseconds} old is issued.
 *
 * <p>After a failure, a new writer gets initialized with one or more states that contain unacknowledged
 * messages. These messages get resend immediately while buffering them in the new state of the writer.
 **/
public class RabbitMQSinkWriterAtLeastOnce<T> extends RabbitMQSinkWriterBase<T> {
    ...
    /**
     * All messages that are older than the minimal resend interval will get resend.
     * A single state is returned that contains just the messages that could not be acknowledged
     * within the last checkpoint.
     *
