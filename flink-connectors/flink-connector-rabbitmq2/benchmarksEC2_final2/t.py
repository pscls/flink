...
def windowThroughput():
    ...
    print("Perc.9: " + str(throughputs[int(len(throughputs)*0.1)]))
    print("Perc.95: " + str(throughputs[int(len(throughputs)*0.05)]))
    print("Perc.999: " + str(throughputs[int(len(throughputs)*0.01)]))
...
values = readDir('atmost_T5_usable')

# values = values[:-1330000]
values = [int(v) for v in values]
values.sort()
# values = [v for v in values if v - values[0] > 5000 and v - values[0] < 36000]
