...
//            FlinkContainer.builder().build().withNetwork(network).dependsOn(rabbitmq);
    ...
    public RabbitMQSourceITCase() throws IOException, TimeoutException {
        ...
        this.rabbitmq = new RabbitMQContainer(DockerImageName.parse("rabbitmq"))
                .withNetwork(network)
                .withNetworkAliases(INTER_CONTAINER_RABBITMQ_ALIAS);
        System.out.println("3");

        FlinkContainer.builder().build().withNetwork(this.network).dependsOn(this.rabbitmq);
